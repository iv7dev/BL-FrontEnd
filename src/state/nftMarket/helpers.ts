...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Genesys Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
const fetchWalletMarketData = async (
  walletNftsByCollection: { collectionAddress: string; idWithCollectionAddress: TokenIdWithCollectionAddress[] }[],
): Promise<TokenMarketData[]> => {
