...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of GSYS up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max GSYS entry is capped by minimum between your average GSYS balance in the IFO GSYS pool, or the pool’s hard cap. To increase the max entry, Stake more GSYS into the IFO GSYS pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max GSYS entry is capped by your average GSYS balance in the IFO GSYS pool. To increase the max entry, Stake more GSYS into the IFO GSYS pool',
        )
      : t(
          'For the unlimited sale, Max GSYS entry is capped by your average GSYS balance in the IFO GSYS pool. To increase the max entry, Stake more GSYS into the IFO GSYS pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. GSYS entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('GSYS to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
